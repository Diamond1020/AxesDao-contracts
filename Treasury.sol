...
contract AXESTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SXYZ }
    ...
    address public sXYZ;
    uint public sXYZQueue; // Delays change to sXYZ address
    ...
    constructor (
       ...
       isLiquidityToken[ _XYZBUSD ] = true;
       liquidityTokens.push( _XYZBUSD );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for XYZ
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint XYZ needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( XYZ ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn XYZ for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IXYZERC20( XYZ ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sXYZ ).balanceOf( msg.sender ); // Can only borrow against sXYZ held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with XYZ
        @param _amount uint
     */
    function repayDebtWithXYZ( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IXYZERC20( XYZ ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, XYZ, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( XYZ ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( XYZ ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns XYZ valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match XYZ decimals
            value_ = _amount.mul( 10 ** IERC20( XYZ ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SXYZ ) { // 9
            sXYZQueue = block.number.add( blocksNeededForQueue );
        } else return false;
